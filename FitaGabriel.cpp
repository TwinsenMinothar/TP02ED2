...
void exemplo(int quantidade)// recebe situacao
{
   FILE* arq;
   ...
   Aluno* alunoBug = new Aluno[quantidade];
   Aluno a;
   ...
   while(controle < quantidade)
   {
      ...
      else
      {
         ...
         /*fseek(fitaEntrada[cont1], -1, SEEK_CUR);
         fread(alunoBug, sizeof(alunoBug), 1, fitaEntrada[cont1]);
         cout << alunoBug[cont1].fita << endl;*/
         cont2 = 0;
         cont1++;
      }
      controle++;    
...
// fechar arquivo
   ...
   // leitura do arquivo passado
   // escrita da struct Aluno de 10 em 10 dados em arquivos binários (fitas de saida)
   // intercala e encerra
}

void intercala(int quantidade) //int situacao
{
   FILE** fitaEntrada = new FILE*[10];
   FILE** fitaSaida = new FILE*[10];
   string nomeArq, temp;
...
// fazer um while que vai ate quantidade
// colocar um contador no else(estado) até 10
// colocar no comeco de cada fita quantos itens ela tem para que no final usarmos essa informacao para controlarmos o codigo